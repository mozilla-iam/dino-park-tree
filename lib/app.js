...
class App {
  ...
  async init(cfg) {
    ...
    const roots = cfg.orgTreeRoots
      .split(",")
      .map((r) => r.trim())
      .filter(Boolean);
    logger.info(`org tree roots: ${roots}`);
    const storage = await new Storage(cfg).init();
    const orgchart = await new Orgchart(storage, DinoTree, roots).init();

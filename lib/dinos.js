...
class Dino {
  constructor(fullProfile) {
    if (fullProfile.staff_information.staff.value && fullProfile.active.value) {
      ...
      this.employeeId =
        fullProfile.access_information.hris.values.primary_work_email;
      this.managerId =
        fullProfile.access_information.hris.values.managers_primary_work_email;
...
class DinoTree {
  ...
  _init(dinos) {
    const roots = _findRoots(this.roots, dinos);
    ...
    const noMans = _findNoMans(this.roots, dinos);
  ...
  fullOrgchart() {
    const forrest = [];
    for (let i = 0; i < this.firstLoose; i++) {
      const r = this.tree[i];
      if (r.parent > -1) {
        break;
      }
  ...
  related(username) {
    ...
    const managerNode = node.parent > -1 ? this.tree[node.parent] : null;
    const peers =
      (managerNode &&
        this._directsData(managerNode).filter(
          (data) => data.username !== username,
        )) ||
      [];
    const directs = this._directsData(node);
    return {
      manager: managerNode && managerNode.dino.data,
      peers,
      directs,
    };
  }
...
function _findRoots(cfgRoots, dinos) {
  const roots = dinos.filter((d) => cfgRoots.includes(d.mail));
  const map = new Map(roots.map((d) => [d.mail, d]));
  return cfgRoots.map((mail) => map.get(mail)).filter((d) => d);
}

function _findNoMans(cfgRoots, dinos) {
  const currentIds = new Set(dinos.map((d) => d.employeeId));
  return dinos.filter(
    (d) =>
      !cfgRoots.includes(d.mail) &&
      (!d.managerId || !currentIds.has(d.managerId)),
  );
}
